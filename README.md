...
10. [Полезные материалы](#полезные-использования)
...
- Пример использования:
    ```html
    <div class="mobile:text-[10px] desktop:text-[22px]">...</div>
    ```
...
- Синтаксис:
    ```html
    <div class="bg-white dark:bg-[#1f1f1f]">...</div>
    ```
...
```vue
<template>
    <Auth :visible="isAuthVisible" @close="handleClose" @success="handleSuccess" />
</template>
...
```vue
<template>
    <NewsBlock :news="newsList" :loading="isLoading" @load-more="handleLoadMore">
        <template #empty>
...
```vue
<template>
    <TeacherCard :teacher="teacherData" :show-details="true" @click="handleTeacherClick">
